...
WIN_W, WIN_H = 560, 720  # must match the windows.lua rule
...
class Sill(Adw.Application):
    ...
    def do_activate(self):
        ...
        self.holder.set_halign(Gtk.Align.END)
        self.holder.set_valign(Gtk.Align.START)
        self.holder.set_margin_end(int(self.settings.get("sill.margin", 10)))
    ...
    def _wire_settings(self):
        s = self.settings
        s.on("sill.margin", lambda v: self.holder.set_margin_end(int(v)))
